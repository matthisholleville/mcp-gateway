...
.PHONY: help run build clean test lint fmt vet deps install serve serve-postgres migrate dev check mocks swagger envrc-sample check-envrc helm-docs create-migration
...
serve-postgres:
	...
	go run main.go serve --log-format=text --log-level=debug --http-admin-api-key=admin --backend-engine=postgres --backend-uri='mcp-gateway:changeme@localhost:5439/mcp-gateway?sslmode=disable' --backend-encryption-key=0123456789abcdeffedcba9876543210cafebabefacefeeddeadbeef00112233

migrate:
	@echo "$(YELLOW)Migrating database...$(RESET)"
	go run main.go migrate --verbose --log-level=info --log-format=json
