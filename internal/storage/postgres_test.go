...
func TestGetURI(t *testing.T) {
	for _, test := range []struct {
		...
		},
		{
			...
			uri: "postgresql://mcp-gateway:postgres@localhost:5439/mcp-gateway?sslmode=disable",
		},
		{
			name: "with password",
			cfg: &cfg.Config{
			...
			uri: "postgresql://postgres:changeme@localhost:5439/mcp-gateway?sslmode=disable",
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			uri, err := getURI(test.cfg)
			assert.NoError(t, err)
